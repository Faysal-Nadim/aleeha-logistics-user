  ...
  useEffect(() => {
    function slideShow() {
      document.getElementById("slider").className +=
        "relative max-h-[588px] h-full rounded-[16px] fadeOut";
      setTimeout(function () {
        document.getElementById("slider").style.backgroundImage = `url(${
          campaign?.campaigns?.hero
            ? campaign?.campaigns?.hero[curIndex]?.img?.prize
            : ""
        })`;
        setSelectedItem(curIndex);
        document.getElementById("slider").className =
          "relative max-h-[588px] h-full rounded-[16px]";
      }, 1000);
